...
public class TiledCollectionClient(
    ...
    int storeZoom = 11)
    : TiledCollectionClient<StoredFeature>(container, loggerFactory, storeZoom)
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var requestedZoom = zoom ?? _storeZoom;
        var requestedKeys = keys.Distinct().ToList();
        var docs = await FetchByTilesCore(requestedKeys, requestedZoom, requestChargeAccumulator, cancellationToken, filter, filterParameters);

        var visibleDocuments = docs.Where(d => !d.Id.StartsWith("empty-")).ToList();
        if (followPointers)
            visibleDocuments = (await ResolvePointers(visibleDocuments, cancellationToken)).ToList();

        return visibleDocuments.OrderBy(StoredFeature.IsPointerDocument).DistinctBy(d => d.LogicalId);
    }
        ...
        IReadOnlyDictionary<string, object?>? filterParameters)
    {
        ...
        return requestedZoom != _storeZoom
